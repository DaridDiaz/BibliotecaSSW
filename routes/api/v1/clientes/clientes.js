...
router.get('/byagegender/:age/:gender', async (req, res) => {
  try {
    ...
    res.status(200).json({ status: 'ok', clientes: row });
  } catch (ex) {
    console.log(ex);
    res.status(500).json({ status: 'failed' });
  }
...
router.post('/new', async (req, res) => {
  ...
  try {
    rslt = await clientemodel.new(nombres, apellidos, identidad, telefono, email);
    res.status(200).json(
      {
        status: 'ok',
        result: rslt
...
router.put('/update/:id', async (req, res) => {
  try{
    const { nombres, apellidos, identidad, email, telefono } = req.body;
    const { id } = req.params;
    const result = await clientemodel.updateOne(id, nombres, apellidos, identidad, telefono, email);
    res.status(200).json({
      status:'ok',
      result
    });
...
router.put('/addtag/:id', async (req, res) => {
  try {
    const { tag } = req.body;
    const { id } = req.params;
    const result = await clientemodel.updateAddTag(id, tag);
    res.status(200).json({
      status: 'ok',
      result
    });
...
router.put('/addtagset/:id', async (req, res) => {
  try {
    const { tag } = req.body;
    const { id } = req.params;
    const result = await clientemodel.updateAddTagSet(id, tag);
    res.status(200).json({
      status: 'ok',
      result
    });
...
router.put('/removetag/:id', async (req, res) => {
  try {
    const { tag } = req.body;
    const { id } = req.params;
    const result = await clientemodel.updatePopTag(id, tag);
    res.status(200).json({
      status: 'ok',
      result
    });
